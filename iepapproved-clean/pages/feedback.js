// pages/feedback.js — empty testimonials, populates with real reviews only
...
// Add real approved 4-5 star reviews here as they come in
...
export default function FeedbackPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.stars) { setError(es ? 'Por favor selecciona una calificación.' : 'Please select a star rating.'); return; }
    if (!form.message.trim()) { setError(es ? 'Por favor escribe tu comentario.' : 'Please write your feedback.'); return; }
    setError('');
    setLoading(true);
    try {
  ...
  return (
    <>
      <Head>
        <title>{es ? 'Comparte Tu Experiencia — IEP Approved' : 'Share Your Experience — IEP Approved'}</title>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
      </Head>
      <Nav />
      <div style={s.page}>
        <div style={s.hero}>
          <p style={s.eyebrow}>{es ? 'TU VOZ IMPORTA' : 'YOUR VOICE MATTERS'}</p>
          <h1 style={s.title}>{es ? 'Comparte Tu Experiencia' : 'Share Your Experience'}</h1>
          <p style={s.subtitle}>
            {es
              ? 'Tu historia ayuda a otras familias a encontrar el apoyo que necesitan.'
              : 'Your story helps other families find the support they need.'}
          </p>
        </div>
        ...
        <div style={s.layout}>
          ...
          <div style={s.formCard}>
            {!submitted ? (
              <>
                <h2 style={s.formTitle}>{es ? 'Tu Comentario' : 'Your Feedback'}</h2>
                {error && <div style={s.errorBox}>{error}</div>}
                <form onSubmit={handleSubmit} style={s.form}>
                  <div style={s.field}>
                    <label style={s.label}>{es ? 'Tu Nombre' : 'Your Name'}</label>
                    <input name="name" type="text" value={form.name} onChange={handleChange}
                      placeholder={es ? 'Nombre (o anónimo)' : 'Name (or Anonymous)'} style={s.input} />
                  </div>
                  <div style={s.field}>
                    <label style={s.label}>{es ? 'Correo Electrónico' : 'Email'} <span style={s.opt}>({es?'opcional':'optional'})</span></label>
                    <input name="email" type="email" value={form.email} onChange={handleChange}
                      placeholder="you@email.com" style={s.input} />
                  </div>
                  <div style={s.field}>
                    <label style={s.label}>{es ? 'Tu Calificación' : 'Your Rating'}</label>
                    <div style={s.starsRow}>
                      {[1,2,3,4,5].map(star => (
                        <button key={star} type="button"
                          ...
                          style={s.starBtn}>
                          <span style={{fontSize:'32px', color: star <= (hoverStar||form.stars) ? '#D4A843' : 'rgba(255,255,255,0.15)', transition:'color 0.15s'}}>★</span>
                        </button>
                      ))}
                      {form.stars > 0 && (
                  ...
                  <div style={s.field}>
                    <label style={s.label}>{es ? 'Tu Comentario' : 'Your Comments'}</label>
                    <textarea name="message" value={form.message} onChange={handleChange}
                      placeholder={es ? 'Cuéntanos cómo IEP Approved o Ada te ha ayudado...' : 'Tell us how IEP Approved or Ada has helped you...'}
                  ...
                  <button type="submit" disabled={loading} style={s.submitBtn}>
                    {loading ? (es?'Enviando...':'Sending...') : (es?'Enviar Mi Comentario':'Submit My Feedback')}
                  </button>
                </form>
              </>
            ) : (
              <div style={s.successWrap}>
                ...
                <h2 style={s.successTitle}>{es ? '¡Gracias!' : 'Thank you!'}</h2>
                <p style={s.successText}>
                  {es ? 'Tu opinión nos ayuda a mejorar y a llegar a más familias.' : 'Your feedback helps us improve and reach more families.'}
                </p>
                <a href="/ada" style={s.backBtn}>{es ? 'Volver a Ada' : 'Back to Ada'}</a>
              </div>
            )}
          </div>

          {/* TESTIMONIALS — only shows when real ones exist */}
          {APPROVED_TESTIMONIALS.length > 0 && (
            <div style={s.testimonials}>
              <h2 style={s.testimonialsTitle}>
                {es ? 'Lo que dicen las familias' : 'What Families Are Saying'}
              </h2>
...
const s = {
  ...
  layout:{maxWidth:'1000px',margin:'0 auto',padding:'0 24px 64px',display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(320px, 1fr))',gap:'32px',alignItems:'start'},
  ...
  backBtn:{display:'inline-block',backgroundColor:'#D4A843',color:'#2D1B4E',padding:'12px 24px',borderRadius:'8px',textDecoration:'none',fontSize:'14px',fontWeight:'800'},
